...
std::string ShaderConvertor::convertShaderToyBuiltins (const std::string& code,
  ...
  if (!usedChannels.empty()) {
    ...
    for (size_t j = 0; j < usedChannels.size(); ++j) {
      uniformDeclaration += "// uniform sampler2D tex" + std::to_string(j) + "; // for " + usedChannels[j] + "\n";
    }
...
std::string ShaderConvertor::fixCompatibilityIssues (const std::string& code, ShaderTarget target) {
  ...
  if (target == ShaderTarget::WebGL1) {
    ...
    result = std::regex_replace(result, floatForRegex, "for(int $1_i = int($2); $1_i < int($3); $1_i++) { float $1 = float($1_i);");
  ...
  if (target == ShaderTarget::WebGL1) {
    ...
    if (std::regex_search(result, mat2Vec4Regex)) {
      ...
      result = "// Warning: Complex mat2 constructor may need manual adjustment for WebGL1\n" + result;
    }
  }
  ...
  if (result.find("vec2 r = iResolution.xy,") != std::string::npos) {
    ...
    result = "// Note: Multiple variable declarations on one line - verify compatibility\n" + result;
  }
...
void ShaderConvertor::initializeFunctionReplacements () {
  ...
  functionReplacements[ShaderTarget::WebGL1]["textureSize"] = "textureSize2D"; // Může vyžadovat extension
...
ShaderConvertor::ShaderAnalysis ShaderConvertor::analyzeShaderCode(const std::string& code) {
  ...
  std::vector<std::string> complexFunctions = {"log", "exp", "pow", "sqrt", "sin", "cos", "atan", "length"};
  ...
  std::regex nonStandardParams(R"(void\s+mainImage\s*\(\s*out\s+vec4\s+(\w+)\s*,\s*(?:in\s+)?vec2\s+(\w+)\s*\))");
  std::smatch match;
  if (std::regex_search(code, match, nonStandardParams)) {
    ...
    if (colorVar != "fragColor" || coordVar != "fragCoord") {
      ...
      analysis.warnings.push_back("Non-standard parameter names: " + colorVar + ", " + coordVar + " (should be fragColor, fragCoord)");
    }
  }
  ...
  if (analysis.hasTextureChannels) {
    analysis.warnings.push_back("ShaderToy texture channels detected - will need manual uniform setup");
  }
  ...
  if (analysis.hasCustomFunctions) {
    analysis.warnings.push_back("Custom function definitions detected - verify parameter conversions");
  }
  
  // Detekce advanced GLSL konstrukcí
  if (containsFunction(code, "mat2(") || 
      containsFunction(code, "mat3(") || 
