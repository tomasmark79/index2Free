...
// WebGL1 doesn't have 'out' qualifier
#define shadertoy_outcolor gl_FragColor

// "Dying Universe" by Martijn Steinrucken aka BigWings - 2015
// License Creative Commons Attribution-NonCommercial-ShareAlike 3.0 Unported License.

vec4 COOLCOLOR = vec4(1.0, 0.5, 0.0, 0.0);
...
float BounceFast(float t) {
    ...
    return max(max(a1, a2), max(a3, 0.0));
}
...
float BounceNorm(float t, float decay) {
    ...
    // WebGL1 doesn't support variable-length arrays, so we use fixed size
    float heights[7]; heights[0] = 1.0; // NUM_ARCS = NUM_BOUNCES + 1 = 7
    float halfDurations[7]; halfDurations[0] = 1.0;
...
vec3 IntersectPlane(ray r) {
    return IntersectPlane(r, vec4(0.0, 1.0, 0.0, 0.0));
}
...
vec4 Star(ray r, float seed) {
    vec4 noise = Noise4(vec4(seed, seed + 1.0, seed + 2.0, seed + 3.0));
    ...
    vec3 sparkPos = vec3(noise.x * 10.0, b, noise.y * 10.0);
...
vec4 CalcStarPos(int i) {
    ...
    vec4 noise = Noise4(vec4(n, n + 1.0, n + 2.0, n + 3.0));
    
    float t = fract(time * 0.1 + n) * 2.0;
    ...
    float b = BounceNorm(t, 0.4 + n * 0.1) * 7.0;
    b += size;
...
vec4 Ground(ray r) {
    ...
    for(int i = 0; i < NUM_STARS; i++) {
        ...
        float light = lambert / pow(dist, 1.0);
        ...
        float fresnel = 1.0 - saturate(dot(L, up));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    float t = time * pi * 0.1;
    COOLCOLOR = vec4(sin(t), cos(t * 0.23), cos(t * 0.3453), 1.0) * 0.5 + 0.5;
    HOTCOLOR = vec4(sin(t * 2.0), cos(t * 2.0 * 0.33), cos(t * 0.3453), 1.0) * 0.5 + 0.5;
